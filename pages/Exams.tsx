...
import { 
  Clock, MapPin, AlertTriangle, Plus, Trash2, Loader2, Copy, Share2, Pencil 
} from 'lucide-react';
...
export default function Exams() {
  ...
  // ... (JSX is identical to provided previous content, just using new logic)
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between sticky top-0 bg-gray-50/95 dark:bg-gray-900/95 py-4 z-10 backdrop-blur-sm">
        <div>
        ...
        {canManage && (
          <button 
            ...
            className="flex items-center gap-2 bg-primary-500 hover:bg-primary-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-md transition-all hover:scale-105"
          >
            <Plus size={18} />
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                 <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className={`text-xl font-bold ${isPassed ? 'text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
                          {exam.subject}
                        </h3>
                    </div>
                    ...
                    {exam.notes && (
                      <p className="text-sm text-gray-500 dark:text-gray-400 italic border-l-2 border-primary-200 dark:border-primary-800 pl-3 mb-4">
                        {exam.notes}
                      </p>
                    )}
                 </div>
                 ...
                 <div className="flex items-center gap-2 self-start border-t md:border-t-0 md:border-l border-gray-100 dark:border-gray-700 pt-4 md:pt-0 md:pl-4">
                    <button onClick={() => handleCopy(exam)} className="p-2 text-gray-400 hover:text-primary-500 transition-colors rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700" title="Copier les détails">
                      <Copy size={18} />
                    </button>
                    <button onClick={() => handleShare(exam)} className="p-2 text-gray-400 hover:text-primary-500 transition-colors rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700" title="Partager par mail">
                      <Share2 size={18} />
                    </button>
                    
                    {canManage && (
                      <>
                        <button onClick={() => handleEdit(exam)} className="p-2 text-gray-400 hover:text-blue-500 transition-colors rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20" title="Modifier">
                          <Pencil size={18} />
                        </button>
                        <button onClick={(e) => handleDelete(e, exam.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20" title="Supprimer">
                          <Trash2 size={18} />
                        </button>
                      </>
                    )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
              ...
              className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-300 outline-none" 
            />
          </div>
          ...
          <button 
            type="submit" 
            disabled={submitting}
            className="w-full bg-primary-500 hover:bg-primary-600 text-white font-bold py-3 rounded-lg flex justify-center items-center gap-2"
          >
